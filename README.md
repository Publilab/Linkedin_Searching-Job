...
- Registrados para futura integración, aún deshabilitados: `trabajando_public`, `laborum_public`, `computrabajo_public`, `trabajosdiarios_public`, `indeed_public`, `glassdoor_public`, `flexjobs_public`, `weworkremotely_public`, `remoteco_public`, `dailyremote_public`.
- Dedupe:
...
- Con LLM y búsqueda corta: `0.52*llm_fit + 0.18*deterministico + 0.08*recencia + 0.10*ubicacion + 0.12*personalizacion`.
- Con LLM y búsqueda larga (`>= 168h`): `0.55*llm_fit + 0.15*deterministico + 0.05*recencia + 0.10*ubicacion + 0.15*personalizacion`.
- Fallback y búsqueda corta: `0.60*deterministico + 0.12*recencia + 0.10*ubicacion + 0.18*personalizacion`.
- Fallback y búsqueda larga (`>= 168h`): `0.55*deterministico + 0.08*recencia + 0.12*ubicacion + 0.25*personalizacion`.
- El límite de saturación por postulantes ahora es configurable por búsqueda (`max_applicant_count`), con default `100`.

## Estrategia de búsqueda

- Las queries ahora se generan por roles y áreas funcionales, no concatenando bloques completos del CV.
- Se priorizan `target_roles`, `secondary_roles`, consultas sugeridas por IA y preferencias aprendidas.
- Se expanden variantes bilingües para roles frecuentes (`Analista de Datos` <-> `Data Analyst`, `Gerente de Proyectos` <-> `Project Manager`, `Recursos Humanos` <-> `Human Resources`, etc.).
- Términos de sector (`sector público`, `public sector`, `sector privado`) se depuran del texto de búsqueda para evitar consultas demasiado rígidas.
